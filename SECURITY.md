...
The Windows lifecycle installs only a manifest-hashed file whitelist under the current user's Local AppData, rejects traversal and filesystem links at owned boundaries, writes state atomically, and refuses to claim or delete a nonempty directory without a valid ownership marker. Install and repair locate a native Codex executable or the standard npm Codex launcher, verify its version output, and persist only absolute shell-free launch metadata; browser input cannot select or modify that command. On-demand activation is a read-back-verified current-user URI registration containing only a fixed start action; it accepts no URL-supplied arguments or authority. Optional startup is a separately verified current-user shortcut. Conflicting registry or shortcut state fails closed and is not removed. Stop uses an installation-specific named pipe and 256-bit control secret; the PID is corroborating metadata and is never used alone to terminate a process.
