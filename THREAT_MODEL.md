...
6. **CLI, URI activation, and environment to local files/processes.** Paths, origins, ports, configuration, PIDs, activation registrations, and lifecycle state may be malformed or manipulated by the current user or another same-user process. Install/uninstall work must validate resolved ownership boundaries before replacement or deletion. Browser-triggered activation is limited to one fixed start action and carries no dynamic command, path, origin, prompt, or secret.
...
The current official App Server contract documents sandbox and approval controls but no per-turn switch that removes all built-in tools. Generated schemas from installed `codex-cli 0.146.0`, current stable `0.147.0`, and `0.148.0-alpha.8` also expose neither restricted readable roots nor a built-in-tool disable control. Process termination after an unsafe event is reactive, so a tool may begin and the read-only sandbox may permit reads outside the working directory before its event is observed. Public Beta publication remains blocked until compatible preventive upstream controls exist, a separate external isolation boundary is approved and verified, the explanation operation is disabled, or the owner explicitly accepts this residual risk. Acceptance permits a release decision but does not remediate the finding.
...
Malformed origins, ports, paths, PID files, activation state, and stale installation state are realistic local inputs. Lifecycle code uses platform APIs or spawned argument arrays, canonicalizes and validates owned paths, avoids shell interpolation, writes restrictive per-user files, identifies the process beyond a reusable PID, recovers from partial installation, and makes uninstall narrow and idempotent. The per-user URI handler starts only the fixed managed connector command and does not pass the invoked URI; a hostile website can at most request startup, not authorize or control the bridge. Registration collisions, unexpected values/subkeys, and ownership mismatches fail closed. Symlinks, junctions, traversal, environment-variable redirection, unsafe archive extraction, registry-command injection, and broad recursive deletion are high-risk installer classes.
