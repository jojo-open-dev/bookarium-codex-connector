...
A repository-wide Codex Security scan is required for each release candidate. Valid findings must be fixed and regression-tested, or recorded as explicit release blockers with an accountable owner decision. The current high-severity blocker is App Server's lack of a documented preventive per-turn switch for all built-in tools. Official documentation now describes optional restricted read roots, but the currently tested installed CLI rejects that newer field. No network, no approvals, an empty working directory, fail-closed item handling, and confirmed process termination reduce impact but do not eliminate the event-observation race or prove filesystem-read isolation.
